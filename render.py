...
.plan-day-today {{ padding:9px; border:2px solid var(--ist); background:var(--wash);
  box-shadow:0 0 0 3px color-mix(in srgb, var(--ist) 10%, transparent); }}
.plan-day-name {{ font-family:var(--mono); font-size:10px; letter-spacing:.08em; text-transform:uppercase; color:var(--muted); }}
.plan-day-today .plan-day-name {{ color:var(--ist); font-weight:700; }}
.plan-today-badge {{ align-self:flex-start; padding:2px 7px; border-radius:999px; background:var(--ist); color:white;
  font-family:var(--mono); font-size:9px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; }}
.plan-today-inline {{ display:inline-block; margin-left:6px; color:var(--ist); font-size:9px; font-weight:700;
...
(function () {{
  ...
  function weekContainsDate(week, value) {{
    return value >= week.weekStart && value <= week.weekEnd;
  }}
  ...
  weekSelect.innerHTML = defaults.weeks.map(function (week) {{
    var suffix = weekContainsDate(week, browserToday()) ? ' · This week' : (week.isRaceWeek ? ' · Race week' : '');
  ...
  function selectWeek(weekStart, announce) {{
    ...
    weekKicker.textContent = weekContainsDate(weekDefaults, browserToday()) ? 'This week' : (weekDefaults.isRaceWeek ? 'Race week' : 'Upcoming week');
  ...
  function renderPlan() {{
    ...
    planCards.innerHTML = plan.days.map(function (day) {{
      ...
      return '<article class="plan-day' + (isToday ? ' plan-day-today' : '') + '"' + (isToday ? ' aria-current="date"' : '') + '>' +
    ...
    planFields.innerHTML = plan.days.map(function (day, index) {{
      ...
      return '<fieldset class="plan-row">' +
        '<legend>' + escapeHtml(day.day) + (isToday ? '<span class="plan-today-inline">Today</span>' : '') + '</legend>' +
  ...
  var actualWeek = defaults.weeks.find(function (week) {{ return weekContainsDate(week, browserToday()); }});
  selectWeek(actualWeek ? actualWeek.weekStart : defaults.selectedWeek, false);
  renderManualRuns();
  function refreshTodayAtMidnight() {{
    var now = new Date();
    var nextMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 0, 0, 1);
